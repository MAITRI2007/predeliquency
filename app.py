...
# Every "Offer Restructuring" click calls this same fixed number,
# regardless of which customer was selected. Must be a number
# verified in your Twilio console while on a trial account.
FIXED_CALL_NUMBER = os.getenv("FIXED_CALL_NUMBER")
...
def prepare_display_df():
    ...
    display_df["income_stress_flag"] = np.where(
        ...
        &
        (
            display_df["current_week_income"]
            <= 0.5
            * display_df["income_5w_avg"]
        )
        &
        (
    ...
    # -----------------------------------------------------
    # Income stress influences risk
    # -----------------------------------------------------

    display_df["status"] = np.where(

        (
            display_df["income_stress_flag"] == 1
        )
        &
        (
            display_df["status"] == "LOW RISK"
        ),
    ...
    # -----------------------------------------------------
    # Gig income influence
    # -----------------------------------------------------

    display_df["status"] = np.where(

        (
            display_df["gig_income_drop_flag"] == 1
        )
        &
        (
            display_df["status"] == "MEDIUM RISK"
    ...
    display_df["primary_risk_reason"] = (
        display_df.apply(
...
def build_reason_chart_data(display_df):
    ...
    if (
        display_df.empty
        or "primary_risk_reason"
        not in display_df.columns
    ):
        return []


    high_risk_df = display_df[
        display_df["status"] == "HIGH RISK"
    ]
...
def build_customer_timeline(
    ...
    if (
        "customer_id" not in display_df.columns
        or
        "year_month" not in display_df.columns
    ):
...
def build_status_summary(display_df):
    ...
    if (
        display_df.empty
        or "status" not in display_df.columns
    ):
...
@app.route("/api/send-email-alert/<customer_id>", methods=["POST"])
def send_email_alert(customer_id):

    # Demo recipient
    receiver_email = "maitruuu5678@gmail.com"

    subject = "Important: Financial Risk Alert – Action Recommended"

    body = f"""
Dear Customer,

We are writing to inform you that our monitoring system has identified certain recent changes in your financial activity that may require your attention.

Customer ID: {customer_id}

Based on our recent assessment, your account has been flagged for elevated financial risk. This may be associated with changes in income patterns, payment behaviour, or other account activity.

To help you avoid potential payment difficulties in the future, we recommend reviewing your current financial obligations and considering the available support options.

Our team may contact you shortly to discuss suitable preventive measures, if required.

Please note that this alert is intended as an early intervention measure and does not indicate that your account is currently in default.

Regards,
BankAI Risk Management Team
Proactive Financial Risk & Customer Support
"""
...
# =========================================================
# TRIGGER RESTRUCTURING CALL (TWILIO)
# =========================================================

@app.route(
    "/api/trigger-call/<customer_id>",
    methods=["POST"]
)
def trigger_call(customer_id):

    if not FIXED_CALL_NUMBER:

        return jsonify({
            "success": False,
            "error": "FIXED_CALL_NUMBER is not set in .env."
        }), 400
